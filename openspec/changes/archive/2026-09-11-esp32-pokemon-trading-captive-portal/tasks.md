...
- [x] 3.4 Update Web UI (`html/index.html`): replace species numeric input with a `<select>` dropdown populated dynamically (1-151 for Gen I, 1-251 for Gen II), auto-populate Mote (Nickname) in uppercase on species change, and update preview.
- [x] 3.5 Update Web UI (`html/index.html`): replace Gen II held item numeric input with a `<select>` dropdown populated with the complete Gen II item list from `src/item_nl.cpp`.
- [x] 3.6 Synchronize UI changes into `include/web_assets.h`, compile with PlatformIO, and upload/verify on hardware.
